...
import {
  ...
  TDExport,
  ImageShape,
} from '~/components/canvasCopy/types'
import {
  migrate,
  FileSystemHandle,
  loadFileHandle,
...
import {defaultStyle} from '~/components/canvasCopy/state/shapes/shared/shape-styles'
...
import {
  ...
  VIDEO_EXTENSIONS,
  SVG_EXPORT_PADDING,
} from '~/components/canvasCopy/constants'
...
export class TldrawApp extends StateManager<TDSnapshot> {
    ...
    if (next.document !== prev.document) {
      Object.entries(next.document.pages).forEach(([pageId, page]) => {
        ...
        if (
          ...
          const bindingsToUpdate = TLDR.getRelatedBindings(
            next,
  ...
  createPage = (id?: string): this => {
    ...
    return this.setState(
      Commands.createPage(this, [-width / 2, -height / 2], id),
    )
  }
    ...
    navigator.clipboard
      ...
      .then(result => {
        ...
        if (data.type === 'tldr/clipboard') {
          ...
          TLDR.warn(
            'The selected shape was not a tldraw shape, treating as text.',
          )
  ...
  copySvg = (ids = this.selectedIds, pageId = this.currentPageId) => {
    ...
    const getSvgElementForShape = (shape: TDShape) => {
      ...
      elm.setAttribute(
        'transform',
        `translate(${
          SVG_EXPORT_PADDING + shape.point[0] - commonBounds.minX
        }, ${SVG_EXPORT_PADDING + shape.point[1] - commonBounds.minY}) rotate(${
  ...
  async exportAllShapesAs(type: TDExportTypes) {
    ...
    const {width, height} = Utils.expandBounds(
      TLDR.getSelectedBounds(this.state),
      64,
    )
