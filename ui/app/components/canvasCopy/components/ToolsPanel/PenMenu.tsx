...
import {TDShapeType, TDToolType} from '~/components/canvasCopy/types'
...
export const PenMenu = React.memo(function PenMenu({
  activeTool,
}: ShapesMenuProps) {
  const app = useTldrawApp()

  const [lastActiveTool, setLastActiveTool] = React.useState<PenShape>(
    TDShapeType.Draw,
  )

  React.useEffect(() => {
    if (
      penShapes.includes(activeTool as PenShape) &&
