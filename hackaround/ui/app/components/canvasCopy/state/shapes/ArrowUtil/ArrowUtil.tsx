...
export class ArrowUtil extends TDShapeUtil<T, E> {
  ...
  onBindingChange = (
    ...
    const handle =
      shape.handles[binding.handleId as keyof ArrowShape['handles']]
    ...
    if (binding.distance) {
      ...
      const direction = Vec.uni(
        Vec.sub(Vec.add(handlePoint, shape.point), origin),
      )
      if (target.type === TDShapeType.Ellipse) {
        ...
        const points = getTrianglePoints(
          ...
          BINDING_DISTANCE,
          target.rotation,
        ).map(pt => Vec.add(pt, target.point))
