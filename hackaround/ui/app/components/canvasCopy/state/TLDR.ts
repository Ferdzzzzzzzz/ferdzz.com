...
export class TLDR {
  ...
  static getBeforeShape<T extends TDShape>(
    shape: T,
    change: Partial<T>,
  ): Partial<T> {
            ...
            shapes: {
              ...Object.fromEntries(
                shapes.flatMap(shape => {
                  const results: [string, Partial<TDShape> | undefined][] = [
                    [shape.id, undefined],
                  ]
  ...
  static onChildrenChange<T extends TDShape>(
    data: TDSnapshot,
    shape: T,
    pageId: string,
  ) {
  ...
  static transformSingle<T extends TDShape>(
    shape: T,
    bounds: TLBounds,
    info: TLTransformInfo<T>,
  ) {
