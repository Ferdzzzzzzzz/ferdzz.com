...
import {SLOW_SPEED, SNAP_DISTANCE} from '~/components/canvasCopy/constants'
...
export class TranslateSession extends BaseSession {
        ...
        selectedShapes
          ...
          .flatMap(shape => {
            return shape.children
              ? [
                  shape,
                  ...shape.children.map(childId => this.app.getShape(childId)),
                ]
              : [shape]
          }),
      ).values(),
  ...
  cancel = (): TldrawPatch | undefined => {
    ...
    if (this.isCreate) {
      ...
      initialShapes.forEach(
        ({id, point}) => (nextShapes[id] = {...nextShapes[id], point}),
      )
